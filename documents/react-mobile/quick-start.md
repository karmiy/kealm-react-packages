...
npm install @kealm/react-mobile --save
...
```tsx | pure
import { Button } from '@kealm/react-mobile'; // 按需引入
...
```ts
import { Button, Dialog } from '@kealm/react-mobile';
import '@kealm/react-mobile/lib/style'; // 引入全部样式
```

### 按需引入样式

```ts
import { Button, Dialog } from '@kealm/react-mobile'; // 按需引入
// 引入需要的样式
import '@kealm/react-mobile/lib/style/button';
import '@kealm/react-mobile/lib/style/dialog';
```
    ...
    module: {
        rules: [
            // 项目代码的样式编译配置
            {
                ...
                use: [...],
                // 排除组件库包
                exclude: [path.join(__dirname, '..', 'node_modules/@kealm/react-mobile')],
            },
            // 组件库的样式编译配置
            {
                test: /\.css$/,
                ...
                // 单独配置仅对组件库的样式进行转换
                include: [path.join(__dirname, '..', 'node_modules/@kealm/react-mobile')]
            }
        ],
    },
};
